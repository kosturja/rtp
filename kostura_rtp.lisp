...
(defun my_find_2 (clause axiom_list)
  ...
  (let ((cnt2 0))
    (loop
	 ...
	 do (progn
	      (print (format t " ~S was resolved with ~S from ~S"(negate clause)
			     item axiom_list))
	      (return cnt2))
	 else do (progn
		   ;(print cnt)
...
(defun my_find (clause axiom_list)
  ...
  ;; since we add one to our counter before we look at an axiom,
  ...
  (let* ((cnt 0)
    ...
    (loop
	 ...
	 do (progn
	      (print (format t "~S was resolved with ~S from axiom ~S from the
knowledge base." (negate clause) clause cnt))
 ...
 (print (format t "We are trying to prove that ~S" clause))
  (let ((new_clause (negate clause))
    ...
    (loop
	 ...
	 do (setf newer_clause (resolve (negate newest_clause) axiom_list
					location))
	...
	(cond
	  ...
	  (t
	   ...
	   (let ((prooflist prflist)
		...
		;; basically the clause we are passed
		...
		(setf temp2 (remove_and_or  (reverse (cdr
						      (reverse prooflist))))))
	       (t
		(setf temp prooflist)))
		...
		;(print (format t "Temp before we resolve:~S" temp))
	     ;; Resolve are clause
	     (setf newer_clause (resolve temp axiom_list (my_find temp
								  axiom_list)))
	     (cond
